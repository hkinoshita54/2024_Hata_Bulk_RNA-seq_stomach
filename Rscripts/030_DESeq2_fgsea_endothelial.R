...
write.table(res,paste0("results/DEG/endothelial/", description, ".txt"), sep ="\t", col.names = T, row.names = F)
...
write.table(fgseaRes, paste0("results/DEG/endothelial/GSEA_H_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)
...
ggsave(paste0("GSEA_H_Up_", description, ".png"), path = "plots/endothelial", width = 3.5, height = 0.5 + nrow(fgseaResUp)/8, units = "in", dpi = 300) 
...
ggplot(fgseaResDn, aes(x = NES, y = reorder(pathway, NES, decreasing = TRUE))) +
  ...
  theme(plot.title = element_text(size = 8, color = "black"),
        axis.title = element_text(size = 8, color = "black"),
        axis.text = element_text(size = 8, color = "black"))
ggsave(paste0("GSEA_H_Dn_", description, ".png"), path = "plots/endothelial", width = 3.5, height = 0.5 + nrow(fgseaResDn)/8, units = "in", dpi = 300) 
...
fgseaRes$leadingEdge <- fgseaRes$leadingEdge %>% lapply(paste, collapse = ",") %>% unlist(recursive = FALSE)
write.table(fgseaRes, paste0("results/DEG/endothelial/GSEA_KEGG_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)
# collapse pathways
...
write.table(fgseaResCollapse, paste0("results/DEG/endothelial/GSEA_KEGG_Collapse_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)


#### run fgsea REACTOME ----
fgseaRes <- fgsea(pathways = collections$REACTOME, stats = rank, eps=0.0, minSize = 10, maxSize = 500)
fgseaRes$leadingEdge <- fgseaRes$leadingEdge %>% lapply(paste, collapse = ",") %>% unlist(recursive = FALSE)
write.table(fgseaRes, paste0("results/DEG/endothelial/GSEA_REACTOME_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)
# collapse pathways
collapsedPathways <- collapsePathways(fgseaRes[order(pval)][padj < 0.25], collections$REACTOME, rank)
fgseaResCollapse <- fgseaRes[pathway %in% collapsedPathways$mainPathways]
write.table(fgseaResCollapse, paste0("results/DEG/endothelial/GSEA_REACTOME_Collapse_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)


#### run fgsea GO:BP ----
fgseaRes <- fgsea(pathways = collections$BP, stats = rank, eps=0.0, minSize = 10, maxSize = 500)
fgseaRes$leadingEdge <- fgseaRes$leadingEdge %>% lapply(paste, collapse = ",") %>% unlist(recursive = FALSE)
write.table(fgseaRes, paste0("results/DEG/endothelial/GSEA_BP_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)
# collapse pathways
collapsedPathways <- collapsePathways(fgseaRes[order(pval)][padj < 0.25], collections$BP, rank)
fgseaResCollapse <- fgseaRes[pathway %in% collapsedPathways$mainPathways]
write.table(fgseaResCollapse, paste0("results/DEG/endothelial/GSEA_BP_Collapse_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)


#### run fgsea C6
fgseaRes <- fgsea(pathways = collections$C6, stats = rank, eps=0.0, minSize = 10, maxSize = 500)
fgseaRes$leadingEdge <- fgseaRes$leadingEdge %>% lapply(paste, collapse = ",") %>% unlist(recursive = FALSE)
write.table(fgseaRes, paste0("results/DEG/endothelial/GSEA_C6_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)
# collapse pathways
collapsedPathways <- collapsePathways(fgseaRes[order(pval)][padj < 0.25], collections$C6, rank)
fgseaResCollapse <- fgseaRes[pathway %in% collapsedPathways$mainPathways]
write.table(fgseaResCollapse, paste0("results/DEG/endothelial/GSEA_C6_Collapse_", description, ".txt"), sep ="\t", col.names = TRUE, row.names = FALSE)


# GSVA ----

### column annotation
type <- c("CD105lo", "CD105hi")
anno_col <- data.frame(row.names = colnames(cts), type = factor(rep(type, each = 2), levels = type))
...
save_pheatmap_pdf(hm, "plots/endothelial/GSVA_DESeq2-vst.pdf")