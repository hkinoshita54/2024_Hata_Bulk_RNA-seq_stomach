...
GeneRank <- read.delim(file = "data/Score_Rank_PTCvsWT.txt")    # input is prepared by excel
...
ggplot(GeneRank, aes(x = rank_PTCvsWT, y = log2FC_PTCvsWT)) +
  geom_point(size = 1, alpha = 1, col = ifelse(GeneRank$log2FoldChange > log2(1.5) & GeneRank$log2FC_PTCvsWT > 0, "red","black")) + 
  geom_text_repel(aes(label = ifelse(log2FoldChange > log2(1.5) & log2FC_PTCvsWT > 0, external_gene_name,"")), colour = "red", size = 3, fontface = "italic") +
  geom_hline(yintercept = 0.0, linetype = "dashed") +
  theme_classic()

ggplot(GeneRank, aes(x = rank_PTCvsWT, y = log2FC_PTCvsWT)) +
  geom_point(size = 1, alpha = 1, col = ifelse(GeneRank$rank_PTCvsWT<=10, "red","black")) + 
  geom_text_repel(aes(label = ifelse(GeneRank$rank_PTCvsWT<=10, external_gene_name,"")), colour = "red", size = 3, fontface = "italic") +
  geom_hline(yintercept = 0.0, linetype = "dashed") +
  xlim(0,20) +
  ylim(0,1.5) +
  theme_classic()
...
ggplot(GeneRank, aes(x = log2FoldChange, y = log2FC_PTCvsWT)) +
  geom_point(size = 1, alpha = 1, col = ifelse(GeneRank$log2FoldChange > log2(1.5) & GeneRank$log2FC_PTCvsWT > 0, "red","black")) + 
  geom_text_repel(aes(label = ifelse(log2FoldChange > log2(1.5) & log2FC_PTCvsWT > 0, external_gene_name,"")), colour = "red", size = 4, fontface = "italic") +
  geom_hline(yintercept = 0.0, linetype = "dashed") +
  theme_classic()

# plot of GeneRank_logFC vs expression (normalized and transformed by vst)
ggplot(GeneRank, aes(x = (PTC1 + PTC2)/2, y = log2FC_PTCvsWT)) +
  geom_point(size = 1, alpha = 1, col = ifelse(GeneRank$log2FoldChange > log2(1.5) & GeneRank$log2FC_PTCvsWT > 0, "red","black")) + 
  geom_text_repel(aes(label = ifelse(log2FoldChange > log2(1.5) & log2FC_PTCvsWT > 0, external_gene_name,"")), colour = "red", size = 4, fontface = "italic") +
  geom_hline(yintercept = 0.0, linetype = "dashed") +
  theme_classic()
